...
next_question.addEventListener("click", () => {
    ...
    if (index !== MCQS.length - 1) {
        ...
        if (correct > 3){ //If user score more than 3
            points.innerHTML = `Congratulations 🎊! You Got ${correct} 💎 Out Of ${MCQS.length}`;
        }
        else if(correct > 1){ //If user score more than 1
            points.innerHTML = `Nice 😎! You Got ${correct} 💎 Out Of ${MCQS.length}`;
        }
        else{ //if user score less than 1
            points.innerHTML = `Sorry 😥! You Got ${correct} 💎 Out Of ${MCQS.length}`;
        }
    }
    for (i = 0; i <= 3; i++) {
